...
enum HTTPMethodConfiguration: URLRequestConvertible {
    ...
    case DELETEDATA(url: URL, body: Data?, additionalHeadersIfNeeded: [String: String]? = nil)
    ...
    var body: Data? {
        ...
        case .DELETEDATA(_, let data, _):                 return data
    ...
    var url: URL {
        ...
        case .DELETEDATA(let url, _, _):                  return url
    ...
    var additionalHeadersIfExist: [String: String]? {
        ...
        case .DELETEDATA(_, _, let additionalHeadersIfNeeded): return additionalHeadersIfNeeded
    ...
    mutating func updateBody(with data: Data) {
        ...
        case .DELETEDATA(let url, _, let additionalHeaders):
            self = .DELETEDATA(url: url, body: data, additionalHeadersIfNeeded: additionalHeaders)
        default:
            return
        }
    }
    ...
    mutating func appending(queryItem: String, value: String?) {
        ...
        case .DELETEDATA(let url, let body, let additionalHeaders):
            self = .DELETEDATA(url: url.appending(queryItem, value: value), body: body, additionalHeadersIfNeeded: additionalHeaders)
    ...
    mutating func append(pathComponent: String) {
        ...
        case .DELETEDATA(let url, let body, let additionalHeaders):
            self = .DELETEDATA(url: url.appendingPathComponent(pathComponent), body: body, additionalHeadersIfNeeded: additionalHeaders)
        case .MultipartPOST(var configuration):
            configuration.modify(
...
struct MultipartRequestConfiguration {
    ...
    private let parameterProperties: [String: Codable]
    ...
    init(url: URL, media: [NetCoreMedia], parameters: [String: Codable]) {
...
//                if data.description?.lowercased() == "войдите в клиентское приложение ещё раз" {
//                    self?.clearDataAndKeychain()
//
...
struct AxasAPI <T: Decodable>: Decodable {
    let message: String?
